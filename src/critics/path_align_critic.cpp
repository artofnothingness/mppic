...
void PathAlignCritic::score(CriticData & data)
{
  ...
  auto evaluate_u = [&P1, &P3, &P2_P1_diff, &P2_P1_norm_sq](
    size_t t, size_t p, size_t s) -> float {
      return ((P3(t, p, 0) - P1(s, 0)) * (P2_P1_diff(s, 0)) +
             (P3(t, p, 1) - P1(s, 1)) * (P2_P1_diff(s, 1))) /
             P2_P1_norm_sq(s);
    };

  static constexpr float eps = static_cast<float>(1e-3);  // meters
  auto segment_short = P2_P1_norm_sq < eps;
  auto evaluate_dist = [&P3](const xt::xtensor_fixed<float, xt::xshape<2>> & P,
      size_t t, size_t p) {
      float dx = P(0) - P3(t, p, 0);
      float dy = P(1) - P3(t, p, 1);
    ...
    for (size_t p = 0; p < trajectories_points_count; p += trajectory_point_step_) {
      ...
      for (size_t s = 0; s < reference_segments_count; s += path_point_step_) {
        ...
        if (segment_short(s)) {
          P[0] = P1(s, 0);
          P[1] = P1(s, 1);
        } else if (float u = evaluate_u(t, p, s); u <= 0) {
          P[0] = P1(s, 0);
          P[1] = P1(s, 1);
        } else if (u >= 1) {
          P[0] = P2(s, 0);
